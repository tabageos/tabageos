...
import { TravelingSceneryThrower } from './TravelingSceneryThrower.js';
		...
		AISceneryThrower.prototype = Object.create(TravelerSkeleton.prototype);
		Object.assign(AISceneryThrower.prototype, MapTraveler.prototype, TravelingSceneryThrower.prototype);
		...
		AISceneryThrower.prototype.thinkAboutWhatToDo = function(sceneryArray, sceneryMap, sceneryCheck, sceneryRemoval, tw, th, smartness, tileCheck) {
			var td = BlitMath.checkTileValueAt(Math.floor(this.x) , Math.floor(this.y) + (this.height/2), sceneryMap, tw || 16 , th || 16, this._hpt);
			...
			if (!this.holding && this.watchingThrow >= this.afterAttackDelay) {
				if (!sceneryCheck(td)) { 
					...
					for (i; i < sceneryArray.length; i++) {
						...
						if(obj._grounded && GeometricMath.testForPointInCircle(this._pos, 360, obj._pos) && obj.y >= this.y - (this.height/2)) {	
					...
					var mtd = TileData.make(this._hpt.x * (tw||16) , this._hpt.y * (th||16) , td);
		...
		AISceneryThrower.prototype.walkOrJump = function(direc, tw, th, closeSceneryX, tileCheck) { 
			...
			var td = BlitMath.checkTileValueAt(this.x + (this._veloc.x >= 0  && (!closeSceneryX || closeSceneryX > this.x) ? (this.width*2) : -this.width), this.y + (this.height/2), this._map, tw || 16 , th || 16);
			var upperTd = BlitMath.checkTileValueAt(this.x + (this._veloc.x >= 0  && (!closeSceneryX || closeSceneryX > this.x) ? (this.width*2) : -this.width), this.y - (this.height/2), this._map, tw || 16 , th || 16);
		...
		AISceneryThrower.prototype.walkJumpOrAttack = function(direc, tw, th, sceneryArray, smartness,tileCheck) { 
			if (this.watchingThrow < this.afterAttackDelay) {
				...
				var td = BlitMath.checkTileValueAt(this.x + (this._veloc.x >= 1 ? (this.width*2) : -this.width), this.y + (this.height/2), this._map, tw || 16 , th || 16);
				var upperTd = BlitMath.checkTileValueAt(this.x + (this._veloc.x >= 1 ? (this.width*2) : -this.width), this.y - (this.height/2), this._map, tw || 16 , th || 16);
