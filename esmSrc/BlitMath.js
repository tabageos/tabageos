...
import { PatternActionEvent } from './PatternActionEvent.js';
	...
	BlitMath._mpoint = new MoverPoint();
	BlitMath._rect = new Rectangle();
    BlitMath._specs = new BlitSpecs(16,16,0);
    ...
    BlitMath.getPositionPoint = function() {
        if (BlitMath.pPointPool.length <= 0) {
            while (BlitMath.pPointPool.length < 50) {
                BlitMath.pPointPool[BlitMath.pPointPool.length] = new MoverPoint();
            }
        }
    ...
    BlitMath.patternBlit = function(subject, source, pattern, indexXMethod, indexYMethod, toPointOffsetX, toPointOffsetY) {
        ...
        var bitRect = new Rectangle();
        var blitPoint = new MoverPoint();
        for (ri = 0; ri < rl; ri++) {
            for (ci = 0; ci < cl; ci++) {
    ...
    BlitMath.patternActionEvent = new PatternActionEvent();
    ...
    BlitMath.dispatchFunctionAssignments = function(eventDispatcher, handlerFunctionString, handlerObject, pattern, tileWidth, tileHeight) {
        if (handlerFunctionString != null) {
            eventDispatcher.addEventListener(PatternActionEvent.FUNCTION_ASSIGNMENT, handlerFunctionString, handlerObject);
            if (BlitMath.patternActionEvent.type == PatternActionEvent.PATTERN_ACTION_EVENT) {
                BlitMath.patternActionEvent = new PatternActionEvent(0,0,0,0,false,PatternActionEvent.FUNCTION_ASSIGNMENT,0,0);
            }
        }
        var a;
        var i = 0;
        ...
        for (ai = 0; ai < alen; ai++) {
            ...
            for (i = 0; i < len; i++) {
                ...
                for (fi = 0; fi < BlitMath.functionAssignments.length; fi++) {
                    ...
                    if ( ((typeof fa != 'number' || typeof fa =='object') && BlitMath.valuesMatch(fa,ob) )  || (typeof fa == 'number' && fa === ob) ) {
                        ...
                        BlitMath.patternActionEvent.typeToBe = PatternActionEvent.FUNCTION_ASSIGNMENT;
        ...
        if (handlerFunctionString != null) {
            eventDispatcher.removeEventListener(PatternActionEvent.FUNCTION_ASSIGNMENT, handlerFunctionString, handlerObject);
        }
    }
    ;
    ...
    BlitMath.convertInternalValues = function(arr, tileWidth, tileHeight, yIndex) {
        ...
        for (i; i < arr.length; i++) {
            var mp = new MoverPoint(i * tileWidth,yIndex * tileHeight);
            var td = new TileData(mp,arr[i]);
            a.push(td);
        }
        return a;
    };
	...
	//updated 1.47
	BlitMath.getLocationOfTile = function(tile, patt, tileWidth, tileHeight, helperPoint) {
        var i = 0;
        var j = 0;
        var tx;
        var ty;
        ...
        for (i = 0; i < l; i++) {
            for (j = 0; j < jl; j++) {
              ...
              //  if ((x >= tx && x < tx + tileWidth) && (y >= ty && y < ty + tileHeight)) {
                    a = patt[i][j];
                    if ( (a[2] && a[3] && a[2] == tile[2] && a[3] == tile[3]) || (a[0] && a[1] && a[0] == tile[0] && a[1] == tile[1]) || a == tile ) {
    ...
    BlitMath.removeTileData = function(td, map, canvasObject, img, tw, th, clearWidth, clearHeight, tdh) {
        ...
        BlitMath._specs.blitWidth = tw;
        BlitMath._specs.blitHeight = th;
        BlitMath.specificBlit(canvasObject, img, map, tw, th, td.position.x / th, td.position.y / tw);
        
        if (!tdh)
            BlitMath.convertToTileDataHolder(map, tw, th);
    };
    ...
    BlitMath.addValueToPosition = function(x, y, value, map, canvasObject, img, tw, th, clearWidth, clearHeight, tdh) {
		...
		if(canvasObject) {
			...
			BlitMath._specs.blitWidth = tw;
			BlitMath._specs.blitHeight = th;
			BlitMath.specificBlit(canvasObject, img, map, tw, th, x / th, y / tw);
