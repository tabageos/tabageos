...
import { BlitMath } from './BlitMath.js';
	...
	'use strict';
            ...
            function SimpleIsoScene(tileWidth, tileHeight, floorCO, canvasObject, player, map, tileSheetImg, playerValue, noPassValue, floorValues,  xOffset, yOffset, spread, xSpreadShifter,ySpreadShifter, camera) {
                ...
                this._helperPoint = new SimpleIsoPoint(0,0,0);
                this._mp = new MoverPoint();
                this._rect = new Rectangle();
                this._rect2 = new Rectangle();
                this._focusTile = new SimpleIsoPoint();
                this._focusPoint = new SimpleIsoPoint();
                ...
                this._effRect1 = new Rectangle();
                this._effRect2 = new Rectangle();
                this._blittOnce = 0;
            };
			...
			SimpleIsoScene.prototype.getPosition = function(tw,th, tx,ty, renderXOffset, renderYOffset, renderSpread) {
                ...
                this._mp.x =  offX  * ((thdtw)/twdth) * SimpleIsoPoint.rawToIso(destX,destY,1,0) + (renderXOffset||this.sceneXOffset);
                this._mp.y =  offY  * ((twdth)/thdtw) * SimpleIsoPoint.rawToIso(destX,destY,0,1) + (renderYOffset||this.sceneYOffset);
                
                this._mp.x -= (tw - this._twidth);
                this._mp.y -= (th - this._theight);
				
				return new MoverPoint(this._mp.x, this._mp.y);
			};
            ...
            SimpleIsoScene.prototype.draw = function(subject, source, value, tw,th, tx,ty, renderXOffset, renderYOffset, renderSpread, aWidthOffset, aHeightOffset, aFromXOffset, aFromYOffset) {
                ...
                if(this._cam) {
                    ...
                    if(GeometricMath.rectanglesOverlapAmount(this._effRect1, this._effRect2)/tw >= 1) {
            ...
            SimpleIsoScene.prototype.checkTileValueAt = function(x,y,patt,tileWidth,tileHeight,indexHolderPoint) {
                ...
                return BlitMath.checkTileValueAt(x,y,patt,tileWidth,tileHeight,indexHolderPoint);
            ...
            SimpleIsoScene.prototype.specificPatternBlit = function(subject, source, pattern, tw, th, renderXOffset, renderYOffset, renderSpread) {
                ...
                for(i; i < l; i++) { j = 0;
                    for(j; j < jl; j++) {
                        ...
                        olad = tw > th ? tw : th;
                        ola = GeometricMath.rectanglesOverlapAmount(this._rect, this._rect2)/olad;
                    ...
                    for (ei; ei < el; ei++) {
                        ...
                        while(drwval[vertCheck] >= 0 && vertCheck <= 15) {
                            ...
                            if(drwval[vertCheck] >= 0) { 
                                ...
                                this._rect.x = sourceX;
                                this._rect.y = sourceY;
                                this._mp.x =  offX  * ((thdtw)/twdth) * SimpleIsoPoint.rawToIso(drwx,drwy,1,0) + (renderXOffset||this.sceneXOffset);
                                this._mp.y =  offY  * ((twdth)/thdtw) * SimpleIsoPoint.rawToIso(drwx,drwy,0,1) + (renderYOffset||this.sceneYOffset)  - ((th/this._upperTileOffset)*(vertCheck/2));
                                if(this._cam) {
                                    this._effRect1.x = this._cam.v.x;
                                    this._effRect1.y = this._cam.v.y;
                                    this._effRect1.width = this._cam.viewPortWidth;
