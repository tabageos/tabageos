...
import { WayDeterminer } from './WayDeterminer.js';
import { BlittedTraveler } from './BlittedTraveler.js';
import { WDTraveler } from './WDTraveler.js';
import { PatternActionEvent } from './PatternActionEvent.js';
...
import { RotatingShooter } from './RotatingShooter.js';
...
import { BasicPlatformer } from './BasicPlatformer.js';
import { WeaponHoldingAttacker } from './WeaponHoldingAttacker.js';
import { BasicNinja } from './BasicNinja.js';
import { AISceneryThrower } from './AISceneryThrower.js';
import { NinjaSceneryThrower } from './NinjaSceneryThrower.js';
import { OneOfMany } from './OneOfMany.js';
import { DungeonGenerator } from './DungeonGenerator.js';
import { ControllerPad } from './ControllerPad.js';
import { seekTouch } from './Utils.js';
import { ResizeGame } from './Utils.js';
import { _pointerEvents } from './Utils.js';
import { loadSpriteSheetAndStart } from './Utils.js';
import { PNGModule } from './PNG.js';//puts PNG on the window
	...
	GameSkeleton.prototype = Object.create(EventDispatcher.prototype);
	...
	GameSkeleton.prototype.initialConstruction = function(specs) {
		...
		this.initialPlayerPosition = specs.initPlayerPosition || new MoverPoint(32,32);
		...
		this._scaleRectRef = new Rectangle();
		...
		this._helperPoint = new MoverPoint();
		this._helperRect = new Rectangle(0,0,this.tileWidth, this.tileHeight);
		...
		this.__specs = specs;window.GameSkeleton = GameSkeleton;
	...
	*/
	GameSkeleton.prototype.initializeLights = function(dim, color, composition) {
		...
		ths._lhelperPoint = new MoverPoint();
		ths._lhelperRect = new Rectangle();
		ths._lightCanvas = new CanvasObject(null,ths.gameWidth, ths.gameHeight);
	...
	GameSkeleton.prototype.changeLightShade = function(dim,color) {
		if(!GameSkeleton.game._lightCanvas) return;
		if(dim || dim === 0) {
			GameSkeleton.game._lightDim = dim;
		}
		if(color) {
			GameSkeleton.game._lightCanvas.context.fillStyle = color;
		}
		
	};
	GameSkeleton.prototype.turnOffLights = function(dim, apply) {
		if(!GameSkeleton.game._lightCanvas) return;
		dim = dim || GameSkeleton.game._lightDim;
		GameSkeleton.game._lightCanvas.context.globalAlpha  = dim;
		GameSkeleton.game._lightCanvas.context.clearRect(0,0,GameSkeleton.game.gameWidth,GameSkeleton.game.gameHeight);
		GameSkeleton.game._lightCanvas.context.globalAlpha  = dim;
		if(dim && dim != 1) {
			GameSkeleton.game._lightCanvas.context.fillRect(0,0,GameSkeleton.game.gameWidth,GameSkeleton.game.gameHeight);
		}
		if(apply) {
			GameSkeleton.game.applyLights();
		}
	};
	...
	GameSkeleton.prototype.lightType = function(number) {
		if(!number) {
			return GameSkeleton.game._lightComp;
		}
		if(number != 1) {
			return 'multiply';
		} else {
	...
	GameSkeleton.prototype.readyLights = function(fromRect,toX,toY, composition) {
		if(GameSkeleton.game._lightCanvas) {
			GameSkeleton.game._lightComp = GameSkeleton.game.lightType(composition);
			GameSkeleton.game.turnOffLights();
			GameSkeleton.game.turnOnLights(fromRect,toX,toY);
			GameSkeleton.game.applyLights();
		}
	...
	GameSkeleton.prototype.animateLights = function(canvasAnimation,toX,toY, speed, composition) {
		if(GameSkeleton.game._lightCanvas) {
			...
			GameSkeleton.game.turnOnLights(canvasAnimation.fromRect,toX,toY);
			GameSkeleton.game.applyLights();
		}
    ...
    GameSkeleton.prototype.createHud = function() {
        ...
        var listnfr = seekTouch() ? (_pointerEvents ? "pointerdown" : "touchstart") : "click";
	...
	GameSkeleton.prototype.valuesMatch = function(v1,v2) {
		return BlitMath.valuesMatch(v1,v2);
	};
	...
	GameSkeleton.prototype.removeCustomHudParts = function() {
		...
		for(i;i<GameSkeleton.game._playerHudParts.length;i++) {
			...
			if(GameSkeleton.game._healthBar.contains(GameSkeleton.game._playerHudParts[i])) {
	...
	GameSkeleton._volumeSliderHandler = function(e) {
		...
		if(!GameSkeleton.game._sliderForOtherThanVolume) {
			...
			GameSkeleton.game.dispatchEvent(    new Event('volumeSliderEvent', (xoff/8)/10 )    );
		}
	};
	...
	GameSkeleton.prototype.initVolumeSliderAnimation = function(x,y, clayer, nox, dontChangeVolume) {
		...
		if(!ths._volumeSliderAnimation) {
			...
			ths._volumeSliderAnimation = new CanvasAnimation(GameSkeleton._volumeSliderBimg,clayer || ths.charLayer,new Rectangle(0,0,(nox ? 80 : 96),16),0,0,(nox ? 80 : 96),16);
	...
	GameSkeleton.prototype.volumeSliderLoop = function(ts) {
		...
		if(ths.controller) {
			...
			if(!GameSkeleton.game._sliderForOtherThanVolume) {
				...
				if(ths.controller.buttons.left && ths._topota > 0.1 ) {ths.controller.buttons.left = 0;
					ths._topota -= .1;GameSkeleton.game.dispatchEvent(    new Event('volumeSliderEvent', ths._topota )    );ths._sliderValue = ths._topota+1-1;
				} 
		...
		if(!GameSkeleton.game._volumeSliderAnimation) { 
			...
			GameSkeleton._volumeSliderFimg.src = GameSkeleton._volumeSliderFront;
			var millis = 15 + 24 * 9 * 7 + 4 + Math.PI + 51;
			ths._volumeSliderAnimation = new CanvasAnimation(GameSkeleton._volumeSliderBimg,ths.charLayer,new Rectangle(0,0,96,16),0,0,96,16);
	...
	GameSkeleton.prototype.setupMouseTouchHandle = function(handleMouseHeldDown) {
		this.mousePoint = new MoverPoint();
	...
	GameSkeleton.prototype.handleMouseTouchMove = function(e) {
       var dmrect = GameSkeleton.game.container.getBoundingClientRect();
		//var dmrect = GameSkeleton.game._scaleRectRef;
        ...
        if(e && e.targetTouches && e.targetTouches.length) {
            ...
            for (i; i < e.targetTouches.length; i++) {
                ...
                GameSkeleton.game.mousePoint.x = tx - dmrect.x + GameSkeleton.game.camera.v.x - GameSkeleton.game.mouseMoveOffset;  
                GameSkeleton.game.mousePoint.y = ty - dmrect.y + GameSkeleton.game.camera.v.y - GameSkeleton.game.mouseMoveOffset;
            }
        } else {
 
            var upmp = MouseController.mouseMoverPoint();
            GameSkeleton.game.mousePoint.x = upmp.x - dmrect.x + GameSkeleton.game.camera.v.x - GameSkeleton.game.mouseMoveOffset;
			GameSkeleton.game.mousePoint.y = upmp.y - dmrect.y + GameSkeleton.game.camera.v.y - GameSkeleton.game.mouseMoveOffset;
	...
	GameSkeleton.prototype.handleMouseDown = function(e) {
		...
		GameSkeleton.game.mouseIsDown = 1;
		
	};
	GameSkeleton.prototype.handleMouseUp = function(e) {
		
		GameSkeleton.game.mouseIsDown = 0;
	...
	GameSkeleton.prototype.basicInitialize = function(containerDivId, rootDivId, displayWidth,displayHeight, cameraWidth,cameraHeight, controllerDivId, useScreenOrganizer, startWidth,startHeight, dontUseSceneChanger, justCreateController) {
			...
			this.root = useScreenOrganizer ? new CanvasObjectContainer(rootDivId, cameraWidth,cameraHeight) : document.getElementById(rootDivId);
			
			this.cameraWidth = cameraWidth;
			this.cameraHeight = cameraHeight;
			this.frameTime = (1000/60) * (60 / this.frameRate ) - (1000/60) * 0.5;
			
			this.backgroundLayer = new CanvasObject(null, displayWidth, displayHeight,0,0,GameSkeleton.game.disableBackgroundAlpha,0);
			
			this.backgroundLayer.drawRect(new Rectangle(0,0,displayWidth, displayHeight), this.backgroundColor);
			if(displayWidth < 4000) {
				this.display = new CanvasObject(null, displayWidth, displayHeight,0,0,0);
			} else {
				this.display = {"placeHolder":1};
			}
			
			this.charLayer = new CanvasObject(null,displayWidth,displayHeight);
			this.cameraLayer = new CanvasObject(null, cameraWidth, cameraHeight,0,0,1);
            ...
            if(controllerDivId && !this.controller) {
                this.controller = new ControllerPad(justCreateController ? null : document.getElementById(controllerDivId));
            ...
            this.camera = new BasicCamera(this.cameraLayer, this.backgroundLayer, this.display, this.charLayer);
			...
			this.cameraPoint = new MoverPoint();
			...
			if(useScreenOrganizer) { 
				this.title = new CanvasObjectContainer(null, cameraWidth,cameraHeight, null, "#ffffff");
				
				this.gameOverContainer = new CanvasObjectContainer(null,cameraWidth,cameraHeight, null, "#ffffff"); 
				...
				this.startButton.addEventListener((seekTouch() ? (_pointerEvents ? "pointerdown" : "touchstart") : "mouseup"), GameSkeleton.game.changeToMainCamera, false);
				...
				if(this.__specs.screenOrganizerType) {
					...
					this.screenOrganizer = new IrisScreenOrganizer(this.root, [this.title, this.cameraLayer, this.gameOverContainer], null);
				}
				this.screenOrganizer.changeScreen(0);
				
				
				if(!dontUseSceneChanger) {
					this.sceneChanger = new TileSceneChanger(this._image, this.display, this.gameWidth,this.gameHeight, this.tileWidth, this.tileHeight, this.player);
			...
			if(!this.soundSystem) {//changes volume back to full when new
				...
				if(GameSkeleton.__preloadSoundSystem != null) {
					this.soundSystem = GameSkeleton.__preloadSoundSystem;
				} else {
	...
	GameSkeleton.prototype.enableAutoPause = function() {
				window.onblur = function(e) {
					if(!GameSkeleton.game.paused) {
						GameSkeleton.game.pause();
					}
					if(GameSkeleton.game._mute == 0) {
						GameSkeleton.game.muteUnmute();
					}
				};
	...
	GameSkeleton.prototype.playSound = function(soundString, poolAmount, stype, sname) {
		...
		if(!this._mute) {
			this.soundSystem.playSound(sname || soundString, 1);
		}
	};
	GameSkeleton.prototype.playMusic = function(soundString, loop, stype, index) {
		if(soundString) {
			this.soundSystem.clearMusicTracks();
			this.soundSystem.addMusic(soundString.indexOf(";base64") == -1 ? (soundString+(stype||".ogg")) : soundString,this.soundSystem._globalVolume,loop === 0 || loop === -1 ? 0 : 1);
		}
		if(!this._mute) {
	...
	GameSkeleton.basicPlatformerSceneryRoutine = function(player, checkToThrowMethod, checkToPickUpMethod, thePickUpMethod, tileValuesArray, sceneryObjects, sceneryObjectLayer, landLayer, source, map, tw, th, maxThrowSpeed, minThrowSpeed, gameWidth, gameHeight, hp, sceneChanger) {
		...
		var i = 0;var obj;var ttw = tw||16; var tth = th||16;var re = false;
						...
						for(i;i < sceneryObjects.length;i++) {//move and interact with sceneryObejcts
							...
							if(obj._canvasAnimation && obj._canvasAnimation.animationSpecs[obj._canvasAnimation.currentAnimation]) {
								obj._canvasAnimation.matchPosition(obj._pos);
								obj._canvasAnimation.animate();
								obj._canvasAnimation.blit();
							} else {
							...
							if(!player.holding && (checkToPickUp) && GeometricMath.rectanglesOverlapAmount(exrec, player.getRectangle())/ttw > 0) {
	...
	GameSkeleton.basicTopDownSceneryRoutine = function(player, checkToThrowMethod, checkToPickUpMethod, thePickUpMethod, tileValuesArray, sceneryObjects, sceneryObjectLayer, landLayer, source, map, tw, th, maxThrowSpeed, minThrowSpeed, gameWidth, gameHeight, hp) {
		...
		var helperPoint = hp || (GameSkeleton.game ? GameSkeleton.game._helperPoint : new MoverPoint());
		
		if(!gameWidth) {
			
			gameWidth = GameSkeleton.game.gameWidth;
		}
		if(!gameHeight) {
			gameHeight = GameSkeleton.game.gameHeight;
		}
		if(!tileValuesArray) {
			tileValuesArray = [];
		}
		var re = false;
                ...
                for(i;i < sceneryObjects.length;i++) {
                    ...
                    if(!obj._grounded) { //_grounded is being used to run this block once.
                        ...
                        if(obj._veloc.x < 1 && obj._veloc.y < 1 && obj._veloc.x > -1 && obj._veloc.y > -1 && !obj._solidSit) {
                            ...
                            objtile = BlitMath.checkTileValueAt(obj.x,obj.y,map,ttw,tth,helperPoint);
                    ...
                    //picking up a SceneryObject that has been picked up before and thrown or that is a RPGSceneryObject added above
                     if(!player.holding && (checkToPickUp) && GeometricMath.rectanglesOverlapAmount(exrec, player.getRectangle())/ttw > 0) {
                        ...
                        sceneryObjects.splice(sceneryObjects.indexOf(obj),1);
                        sceneryObjectLayer.context.clearRect(obj.x,obj.y,ttw,tth);
                        objtile = BlitMath.checkTileValueAt(obj.x,obj.y,player._map,ttw,tth,helperPoint);
                        if(objtile[0] != 0 || objtile[1] != 0) {
                    ...
                    } else if (!player.holding && (!checkToPickUp) && GeometricMath.rectanglesOverlapAmount(exrec, player.getRectangle())/ttw > 0) {
                ...
                var tileRight = BlitMath.checkTileValueAt(player.x + svn,player.y,map,ttw,tth, helperPoint);
                var tileLeft = BlitMath.checkTileValueAt(player.x - 2,player.y,map,ttw,tth, helperPoint);
                var tileUp = BlitMath.checkTileValueAt(player.x ,player.y - 2,map,ttw,tth, helperPoint);
                var tileDown = BlitMath.checkTileValueAt(player.x,player.y + svn,map,ttw,tth, helperPoint);
				...
				for(i; i < tileValuesArray.length; i++) {
					...
					if(BlitMath.valuesMatch(tileValuesArray[i],tileRight)) {
						
						tileNextTo = tileRight; break;
					}
					if(BlitMath.valuesMatch(tileValuesArray[i],tileLeft)) {
						
						tileNextTo = tileLeft; break;
					}
					if(BlitMath.valuesMatch(tileValuesArray[i],tileUp)) {
						
						tileNextTo = tileUp; break;
					}
					if(BlitMath.valuesMatch(tileValuesArray[i],tileDown)) {
                ...
                if(!player.holding && tileNextTo && checkToPickUpMethod() && BlitMath.valuesMatch(player._map[helperPoint.y][helperPoint.x], tileNextTo) ) {//because of checkTileValueAt behavior, we can check all the values together
					//create a TileData Object to be picked up.
                    var td = TileData.make(helperPoint.x*ttw,helperPoint.y*tth,tileNextTo);//and still we know that helperPoint is only set to the matching value found.
					//picking up TileData.
                    player.pickUpTileData(td);
					//remove TileData from the currentMap, this method also redraws for the spot the tile was at.
                    BlitMath.removeTileData(td,map,landLayer,source,ttw,tth,gameWidth,gameHeight,1);
	...
	GameSkeleton.seeAndChaseRoutine = function(obj,enemies,player,helperPoint,chaseRadius, dontOnlyChaseToX, separationDistance, map,tw,th) {
		...
		if(map) {
			...
			//rightTile = BlitMath.checkTileValueAt(obj.x + obj.width + 4,obj.y+obj.height-2,map,tw,th);
			//leftTile = BlitMath.checkTileValueAt(obj.x - 4,obj.y+obj.height-2,map,tw,th);
			...
			brght = BlitMath.checkTileValueAt(obj.x + obj.width + 4,obj.y+obj.height+2,map,tw,th);
			blft = BlitMath.checkTileValueAt(obj.x - 4,obj.y+obj.height+2,map,tw,th);
		...
		if(GeometricMath.testForPointInCircle(obj._pos,chaseRadius || (128),player._pos) && !obj._pRight && !obj._pLeft) {
	...
	GameSkeleton.prototype.maybeStartGame = function(e) {
		...
		if(GameSkeleton.game.screenOrganizer.currentScreen === 0) {
			//title screen
			GameSkeleton.game.changeToMainCamera(e);
		} 
		if(GameSkeleton.game.screenOrganizer.currentScreen === 2) {
			//game over screen
			GameSkeleton.game.hudExit();
		}
		if(GameSkeleton.game.screenOrganizer.currentScreen === 1) {
			
			GameSkeleton.game.pause(e);
		}
	...
	GameSkeleton.prototype.changeToMainCamera = function(e) {
		...
		GameSkeleton.game.startButton.removeEventListener((seekTouch() ? (_pointerEvents ? "pointerdown" : "touchstart") : "mouseup"), GameSkeleton.game.changeToMainCamera, false);
		
		if(GameSkeleton.game.screenOrganizer) {
			GameSkeleton.game.screenOrganizer.addEventListener(ScreenChangeEvent.COVER, "startGameLoop", GameSkeleton.game);
	...
	GameSkeleton.prototype.startGameLoop = function(gameFunc) {
		GameSkeleton.game.screenOrganizer.removeEventListener(ScreenChangeEvent.COVER, "startGameLoop", GameSkeleton.game);
	...
	GameSkeleton.prototype.changeGameSpeed = function(to) {
		TimeKeeper._sae = (1000 / to) - Math.floor(1000/to); 
	};
	...
	GameSkeleton.prototype.pause = function(e) {
		...
		if(GameSkeleton.game) {
			GameSkeleton.game.dispatchEvent(new Event("pauseEvent"));
		}
	};
			...
			if(levelCompleteTime && !GameSkeleton.game._endingLevel && GameSkeleton.game._setATimeO == 0) {  GameSkeleton.game._setATimeO = 1;
				window.setTimeout( function() {GameSkeleton.game._setATimeO = 0;
					if(GameSkeleton.game._doAlternate && !GameSkeleton.game._endingLevel) {
						...
						GameSkeleton.game.screenOrganizer.addEventListener(ScreenChangeEvent.COVER, "_changeDoAlternate", GameSkeleton.game);
	...
	GameSkeleton.prototype._changeDoAlternate = function(e) {
		GameSkeleton.game.screenOrganizer.removeEventListener(ScreenChangeEvent.COVER, "_changeDoAlternate", GameSkeleton.game);
	...
	GameSkeleton.prototype.gameOver = function(onlyRestPositionOnLooseLife, waitTime) { 
			if(GameSkeleton.game.lives > 1) {
				...
				//this.lives = this._initLives+1-1;
					...
					GameSkeleton.game.screenOrganizer.addEventListener(ScreenChangeEvent.SCREEN_CHANGE, "endLevel", GameSkeleton.game);
	...
	GameSkeleton.prototype.reset = function(e) {
			...
			if(e && e.keyCode == 82) {
				window.cancelAnimationFrame(GameSkeleton.game._aid);
				if(GameSkeleton.game.screenOrganizer) { 
					GameSkeleton.game.screenOrganizer.addEventListener(ScreenChangeEvent.COVER, "resetPosition", GameSkeleton.game);
			...
			if(e && e.keyCode == 299) {  
				...
				if(GameSkeleton.game.screenOrganizer) {
					GameSkeleton.game.screenOrganizer.addEventListener(ScreenChangeEvent.COVER, "resetScene", GameSkeleton.game);
	...
	GameSkeleton.prototype.resetPosition = function(e) {
			...
			if(GameSkeleton.game.screenOrganizer) {
				GameSkeleton.game.screenOrganizer.removeEventListener(ScreenChangeEvent.COVER, "resetPosition", GameSkeleton.game);
			}
	...
	GameSkeleton.prototype.resetScene = function(e) {
			...
			if(GameSkeleton.game.screenOrganizer) {
				GameSkeleton.game.screenOrganizer.removeEventListener(ScreenChangeEvent.COVER, "resetScene", GameSkeleton.game);
			}
	...
	GameSkeleton.prototype.endLevel = function(e) {
			...
			if(GameSkeleton.game.screenOrganizer) {
				GameSkeleton.game.screenOrganizer.removeEventListener(ScreenChangeEvent.COVER, "endLevel", GameSkeleton.game);
				GameSkeleton.game.screenOrganizer.removeEventListener(ScreenChangeEvent.SCREEN_CHANGE, "endLevel", GameSkeleton.game);
			}
	...
	GameSkeleton.prototype.makeButton = function(name, x,y,width,height,methodCallString,altString,linkString, forTitle) {
		...
		if(methodCallString && methodCallString.indexOf && methodCallString.indexOf("(") == -1) {
			window.console.log( "the makeButton methodCallString param may need to have the 'methodName()' format with parens" );
		}
		bttn._gameParent = this;

		if(!linkString && !methodCallString.indexOf && typeof methodCallString != "string") {

				bttn.removeEventListener("click", methodCallString, false);
				bttn.addEventListener("click", methodCallString, false);
	...
	GameSkeleton.prototype.setupStandardButtons = function(pauseX,pauseY,pw,ph,homeX,homeY,hw,hh,muteX,muteY,mw,mh,linkX,linkY,lw,lh, linkaltstring, thelink) {
		...
		if(!this.getButton("pauseButton") && !this.getButton("muteButton") && !this.getButton("homeButton") && !this.getButton("linkButton")) {
			...
			this.makeButton("pauseButton", pauseX, pauseY, pw || 16, ph || 16, "GameSkeleton.game.pause()", "pause unpause");
			
			this.makeButton("homeButton", homeX, homeY, hw || 16, hh || 16, "GameSkeleton.game.fullResetToTitle()", "exit to title");
			
			this.makeButton("muteButton", muteX, muteY, mw || 16, mh || 16, "GameSkeleton.game.muteUnmute()", "mute unmute");
	...
	GameSkeleton.prototype.setupLevelSelect = function(lsButtonX,lsButtonY,lsButtonWidth,lsButtonHeight, amountOfLevels, titleScreenFromRect, setupMethodString, dontAppend) {
		...
		this.makeButton("levelSelect", lsButtonX, lsButtonY, lsButtonWidth,lsButtonHeight, "GameSkeleton.game.showLevelSelect()", "Level Select");
	...
	GameSkeleton.prototype.showLevelSelect = function(latestLevel) {
		...
		if(this.__lsShown == 0) {
			...
			var lsBackTo = new MoverPoint(this.cameraWidth/2 - (352/2), this.gameHeight/2 - (240/2));
			if(GameSkeleton.game.screenOrganizer && GameSkeleton.game.screenOrganizer.currentScreen == 0) { //title
				ti = 1;
				cplyr = this.title.floor;
				this.title.floor.copyPixels(this.__levelSelectSpriteSheet, new Rectangle(48,144,352,240), lsBackTo);
				this.removeStartButton();
				
			} else {
				ti = 0;this.__lsShown = 2;lsBackTo.x += this.camera.v.x; lsBackTo.y += this.camera.v.y;
				cplyr = this.charLayer;
				this.charLayer.copyPixels(this.__levelSelectSpriteSheet, new Rectangle(48,144,352,240), lsBackTo);
			...
			for(i; i <= this._levelSelectAmount; i+=1) { 
				...
				cplyr.copyPixels( this.__levelSelectSpriteSheet, new Rectangle(416 + (row*32),176 + (col*48),32,48), new MoverPoint(lsBackTo.x + 16 + (row*32), lsBackTo.y + 32 + (col*48) ) );
	...
	GameSkeleton.prototype._selectLevel = function(e) {
		...
		BlitMath.specificPatternBlit(ths.display, ths._image, ths.sceneChanger.currentMap);//drawing of the level.
		BlitMath.dispatchFunctionAssignments(ths,ths.addedInitializationMethodString,ths,ths.sceneChanger.currentMap,ths.tileWidth,ths.tileHeight);
		
		GameSkeleton.game.changeToMainCamera();
	...
	GameSkeleton.prototype.hideLevelSelect = function() {
		...
		if(!this.title.div.contains(this.startButton)) { ti = 1;
			this.title.floor.copyPixels(this._image, this.__titleScreenRect, new MoverPoint());
			this.title.div.appendChild(this.startButton);
		}
	...
	GameSkeleton.prototype.placeLevelSelectDivs = function(ontitle) {
		...
		for(i;i<=l;i++) {
			...
			if(ontitle && this.__levelSelectDivs[i] && !this.title.div.contains(this.__levelSelectDivs[i])) {
				this.__levelSelectDivs[i].removeEventListener("click", GameSkeleton.game._selectLevel, false);
				this.__levelSelectDivs[i].addEventListener("click", GameSkeleton.game._selectLevel, false);
	...
	GameSkeleton.prototype.localSave = function(saveName, saveData) {
		...
		try {
			...
			GameSkeleton.game.saveError = "";
			
		} catch(e) {
			
			GameSkeleton.game.saveError = e.message;
			ret = 0;
		}
	...
	GameSkeleton.prototype._reEstablish = function(e) {
		...
		GameSkeleton.game.screenOrganizer.removeEventListener(ScreenOrganizer.COVER,"_reEstablish", GameSkeleton.game);
	...
	GameSkeleton.prototype._theReset = function(dontEstablish, terminateWorker) {
			...
			this.container = null; ScreenOrganizer._instance = null;
	...
	GameSkeleton.prototype.removeHUD = function() {
        var listnr = seekTouch() ? (_pointerEvents ? "pointerdown" : "touchstart") : "click";
	...
	GameSkeleton.__binaryToCol = function(name, innerName, w, h) {
		...
		GameSkeleton.__bnCords = new Rectangle(0,0,w,h);
			...
			if(xmlhtt.status == 200 && xmlhtt.readyState == 4) {
				...
				.then(function(ip) {
					ip.file(innerName).async("string").then(function(str) {
						...
						GameSkeleton.__buildColors(GameSkeleton.__bnCords.width,GameSkeleton.__bnCords.height,d);
					}) 
				});
			}
		}
	...
	GameSkeleton.__afterWorkerPreload = function() {
		...
		if(GameSkeleton.__eProgEvent) {
			var i = 0; var l = GameSkeleton.__eProgEvent.potato.length;var pl;
				...
				GameSkeleton.__eProgEvent.totalLoaded = (tot / ft) || GameSkeleton.__eProgEvent.totalLoaded;
				...
				if(allone >= 1) {
					...
					setTimeout(GameSkeleton.__afterWorkerPreload, 50);
				}
	...
	GameSkeleton.__eventBuildColors = function() { 
		try {
			if(!GameSkeleton.__eventBuildCA) {
				GameSkeleton.__eventBuildCA = new CanvasObject(null,GameSkeleton.__eventBuildObject.w, GameSkeleton.__eventBuildObject.h);
			}
			...
			GameSkeleton.__streamlineWorker.addEventListener("message", function(e) { 
				if(e.data.imd) { 
					...
					window.setTimeout(function() {dt = null;GameSkeleton.__eventBuildObject = null;}, 2500);
				...
				if(e.data.prog) {  
					if(!GameSkeleton.__eProgEvent) {
						var loadDataArray = new Array(GameSkeleton._totalLoad);
						loadDataArray[GameSkeleton._totalLoad-1] = e.data.prog;
						GameSkeleton.__eProgEvent = new Event("loading", loadDataArray);
						GameSkeleton.__eProgEvent.totalLoaded = e.data.prog;
						if(GameSkeleton.loadingDispatcher) {
							GameSkeleton.loadingDispatcher.dispatchEvent(GameSkeleton.__eProgEvent);
						}
						
					} else {
						GameSkeleton.__eProgEvent.potato[GameSkeleton._totalLoad-1] = e.data.prog;
						...
						GameSkeleton.__eProgEvent.totalLoaded = (tot / ft) || 0;
			...
			if(GameSkeleton.__streamlineWorker) {
	...
	GameSkeleton.__buildColors = function(w,h,c) {//this brute force method is not used.
		var i,j,d;
		try {
			var ca = new CanvasObject(null,w,h);
	...
	GameSkeleton.__initializei = function(w,h,d) {
		...
		GameSkeleton.__sprites.context.putImageData(d,0,0);
		var ev = document.createEvent("MouseEvents");
		ev.initEvent("GameSkeleton", true, true);
		window.dispatchEvent(ev);
	};
	...
	GameSkeleton.__soundLoadLoop = function() {
		var buffs = GameSkeleton.__buffs;
		var i = 0; var buff;var curl = 0;var sload = 0;
		if(!GameSkeleton.__eProgEvent) {
			var loadDataArray = new Array(GameSkeleton._totalLoad);
			
			loadDataArray[GameSkeleton._totalLoad-1] = sload;
			
			GameSkeleton.__eProgEvent = new Event("loading", loadDataArray);
			GameSkeleton.__eProgEvent.totalLoaded = 0;
		}
		...
		for(i; i < buffs.length; i++) {
			...
			curl = GameSkeleton.__eProgEvent.potato[ GameSkeleton.__requests.indexOf(buff) ];
			if (!curl || curl < 1) {
				GameSkeleton.__eProgEvent.potato[ GameSkeleton.__requests.indexOf(buff) ] = sload;
			}
			...
			if(curl >= 1) {
				GameSkeleton.__buffs.splice(GameSkeleton.__buffs.indexOf(buff),1); break;
			}
			
		}
		
		if(GameSkeleton.__buffs.length) {
			GameSkeleton.__sll = setTimeout(GameSkeleton.__soundLoadLoop, 25);
		} else {
			GameSkeleton.__sll = null;
		}
	...
	GameSkeleton.loadByXhr = function(itemloc, soundn, stype, music) {
		...
		try {
			...
			if(soundn) { 
				var ss = GameSkeleton.__preloadSoundSystem ? GameSkeleton.__preloadSoundSystem : null;
				if( ss && ss._soundNames.indexOf(soundn) === -1 ) {
					...
					GameSkeleton.__requests.push(newSoundBuffer);
					GameSkeleton.__reqDest.push(function() {});
					var ptFunc = function(e) {
						GameSkeleton.__eProgEvent.potato[ GameSkeleton.__requests.indexOf(e.target) ] = 1;
						e.target.removeEventListener('canplaythrough', ptFunc,false);
					};
					newSoundBuffer.addEventListener('canplaythrough', ptFunc, false);
					
					GameSkeleton.__buffs.push(newSoundBuffer);
					if(!GameSkeleton.__sll || GameSkeleton.__buffs.length <= 1) {
						GameSkeleton.__soundLoadLoop();
					}	
				}
				...
				if(stloc == 'h' || stloc == 'l') {
					...
					xhr.addEventListener("load", GameSkeleton.completeXhr);
					xhr.addEventListener("progress", GameSkeleton.progressXhr);
					xhr.send();
					GameSkeleton.__requests.push(xhr);
				} else {
					
					if(!GameSkeleton.__eProgEvent) {
						var loadDataArray = new Array(GameSkeleton._totalLoad);
						
						loadDataArray[GameSkeleton._totalLoad-1] = 1;
						
						GameSkeleton.__eProgEvent = new Event("loading", loadDataArray);
						GameSkeleton.__eProgEvent.totalLoaded = 0;
					}
					GameSkeleton.__requests.push({'response':itemloc});
					return false;
	...
	GameSkeleton.progressXhr = function(e) {
		...
		if(!GameSkeleton.__eProgEvent) {
			var loadDataArray = new Array(GameSkeleton._totalLoad);
			loadDataArray[GameSkeleton.__requests.indexOf(e.target)] = (e.loaded / e.total);
			GameSkeleton.__eProgEvent = new Event("loading", loadDataArray);
			GameSkeleton.__eProgEvent.totalLoaded = 0;
			if(GameSkeleton.loadingDispatcher) {
				GameSkeleton.loadingDispatcher.dispatchEvent(GameSkeleton.__eProgEvent);
			...
			GameSkeleton.__eProgEvent.potato[GameSkeleton.__requests.indexOf(e.target)] = (e.loaded / e.total);
	...
	GameSkeleton.assignPreloadMethod = function(meth) {
		...
		GameSkeleton.loadingDispatcher = new EventDispatcher();
		var seep = {};
		seep.seeProgress = meth;
		GameSkeleton.loadingDispatcher.addEventListener('loading', 'seeProgress', seep);
		
	};
	
	GameSkeleton.completeXhr = function(e) {
		
			if(typeof GameSkeleton.__reqDest[ GameSkeleton.__requests.indexOf(e.target) ] != 'function') {
				var allone = 1;
				if(GameSkeleton.__eProgEvent && GameSkeleton.__eProgEvent.potato) {
					var i = 0; var l = GameSkeleton.__eProgEvent.potato.length;var pl;
				...
				if(allone == 1) {
					...
					if(GameSkeleton.loadingDispatcher && GameSkeleton.__eProgEvent) {
						GameSkeleton.loadingDispatcher.dispatchEvent(GameSkeleton.__eProgEvent);
					}
					
					var toblob = typeof e.target.response == 'string';
					
					GameSkeleton.__reqDest[ GameSkeleton.__requests.indexOf(e.target) ].src = (toblob == true) ? e.target.response : URL.createObjectURL(e.target.response);
					
					GameSkeleton.__requests[ GameSkeleton.__requests.indexOf(e.target) ] = 0;
					
					//URL.revokeObjectURL(unblobed);
					
				} else {
					setTimeout(GameSkeleton.completeXhr, 50, e);
				}
	...
	GameSkeleton.__loadingOfMusic = function() {
		...
		//game is in under construction, last line is about to happen in constructor
					...
					if(GameSkeleton.game && GameSkeleton.game.__specs.preloadMusic) {
						if(!GameSkeleton.__preloadSoundSystem) GameSkeleton.__preloadSoundSystem = new SoundSystem();//the loading soundsystem gets applied as this.soundSystem once basicInitialize is called.
						var i = 0; var l = GameSkeleton.game.__specs.preloadMusic.length;
						for(i; i < l; i++) {
							
							var inis = GameSkeleton.game.__specs.preloadMusic[i];
							var stype = inis.indexOf(".") != -1 ? ("."+inis.split(".")[1]) : ".ogg";
							var nme = inis.replace(stype,"");
							
							GameSkeleton.loadByXhr(0,nme,stype, 1);//when all loading is complete the src will be set to the image loaded, either by xhr or streamline.
	...
	GameSkeleton.__loadingOfSounds = function() {
		...
		//game is in under construction, last line is about to happen in constructor
					//.game has been set to 'this' but only specs have been defined, no methods a loose this at this point
					if(GameSkeleton.game && GameSkeleton.game.__specs.preloadSounds) {
						GameSkeleton.__preloadSoundSystem = new SoundSystem();//the loading soundsystem gets applied as this.soundSystem once basicInitialize is called.
						var i = 0; var l = GameSkeleton.game.__specs.preloadSounds.length;
						for(i; i < l; i++) {
							
							var inis = GameSkeleton.game.__specs.preloadSounds[i];
							var stype = inis.indexOf(".") != -1 ? ("."+inis.split(".")[1]) : ".ogg";
							var nme = inis.replace(stype,""); 
							
							GameSkeleton.loadByXhr(0,nme,stype);//when all loading is complete the src will be set to the image loaded, either by xhr or streamline.
							//that causes basicInitialize to be called.
	...
	GameSkeleton.__checkFinishPreloadFromStreamline = function() {
				...
				if(allone == 1) {
					...
					setTimeout(function() {
						GameSkeleton.__requests = null;
						GameSkeleton.__reqDest = null;
					}, 1500);
					
				} else {
					
					setTimeout(GameSkeleton.__checkFinishPreloadFromStreamline, 50);
	...
	GameSkeleton.establish = function(gameInstance, spriteSheetImage,containerDivId, rootDivId, controllerDivId, gameScale, useScreenOrganizer, startWidth,startHeight) {
			...
			if(spriteSheetImage) {
				...
				if(spriteSheetImage != "streamline" && !GameSkeleton.__sprites) {
					...
					GameSkeleton.__loadingOfSounds();
					GameSkeleton.__loadingOfMusic();
					
					GameSkeleton._totalLoad += 1;
					GameSkeleton.__reqDest.push(  GameSkeleton.img );
					var aload = GameSkeleton.loadByXhr(spriteSheetImage);
					if ( !aload ) {
						//offline
						GameSkeleton.completeXhr( {target:GameSkeleton.__requests[GameSkeleton._totalLoad-1]}  );
					}
					...
					if(GameSkeleton.game.__specs.preloadSounds || GameSkeleton.game.__specs.preloadMusic) {
						
						var m1 = GameSkeleton.game.__specs.preloadSounds ? GameSkeleton.game.__specs.preloadSounds.length : 0;
						var m2 = GameSkeleton.game.__specs.preloadMusic ? GameSkeleton.game.__specs.preloadMusic.length : 0;
						GameSkeleton.__eProgEvent.totalLoaded = ( m1 + m2 - 1 ) / ( m1 + m2 + 1 );
					}
					
					GameSkeleton.__checkFinishPreloadFromStreamline();
