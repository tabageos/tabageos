...
'use strict';
    ...
    CanvasAnimation.prototype.changeDirectionAnimation = function(left, right, up, down, keepAniIndex, noIdle) {
	...
	CanvasAnimation.prototype.finishedCurrentAnimation = function() {
		if(!this.animationSpecs[this.currentAnimation]) {
			...
			return ( this.onlyHorizontalAnimations ? (Math.round(this.ani) >= this.animationSpecs[this.currentAnimation][1].length - 1) : (Math.round(this.ani) >= this.animationSpecs[this.currentAnimation][1].length - 3)  );
		}
	};
